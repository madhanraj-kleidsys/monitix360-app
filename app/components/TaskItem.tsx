...
const TaskItem = React.memo<TaskItemProps>(
  ({ task, onStart, onPause, onStop, onStatusChange }) => {
    const [elapsed, setElapsed] = useState(task.elapsed_seconds || 0);
    const [timings, setTimings] = useState([]);
    const [loadingTimings, setLoadingTimings] = useState(false);
    const [expandedSection, setExpandedSection] = useState<'activity' | 'details' | null>(null);
    ...
    const formatPlannedDate = (d: any): string => {
      const m = moment(d);
      return m.isValid()
    ...
    const formatTimingTime = (d: any): { date: string; time: string } => {
      const m = moment(d);
      return m.isValid()
    ...
    const renderTimingItem = (timing: any) => {
